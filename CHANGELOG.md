...
* Détails :
  ...
  - Deux montées corrigent une faille : **cryptography 45.0.7 → 50.0.1** (oracle de Bleichenbacher sur `pkcs7_decrypt_der`) et **mistune 3.2.1 → 3.3.4** (déni de service quadratique dans `parse_link_text`, CWE-400). Toutes deux hors chaîne de production.
