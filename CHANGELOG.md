...
* Détails :
  ...
  - **Deux formules datées, et aucun paramètre nouveau.** Le changement de régime est porté par le code, non par les données : `cnrps_indemnites_familiales` reçoit une seconde formule, `formula_1989_01_01`. La formule de base vaut jusqu'au 31 décembre 1988 et applique le taux du quatrième rang à chaque enfant au-delà du troisième — le décret n° 86-611 fixe quatre taux, et aucun texte lu ne limite alors le nombre d'enfants. La formule du 1er janvier 1989 réserve ce taux aux enfants que la loi n° 88-39 excepte. Chacune est documentée par son texte dans sa *docstring*. Un paramètre est une **valeur** du droit — un taux, un montant, un âge ; une bascule de régime n'en est pas une, et l'écrire en booléen aurait obligé la formule à interroger les données pour choisir son propre comportement.
  ...
  - Tests : `tests/formulas/test_indemnites_familiales_droits_acquis.yaml` porte **treize cas** en YAML — la bascule au mois près (décembre 1988 contre janvier 1989, sur le même foyer de quatre enfants : 24,400 puis 19,700), les deux situations qui y échappent (droit acquis, enfant handicapé), le régime antérieur en 1987, le régime actuel en 1990 et 2024, le cas courant de trois enfants, et la borne qui empêche de déclarer plus d'enfants qu'il n'y en a. Les quatre tests existants de `test_accessoires.yaml` passent **sans modification** : ils portent sur un et trois enfants, que la limitation ne touche pas. `make test` : 129 tests passent, contre 116 sur `master`.
