...
def get_versions():
    ...
    openfisca_tunisia = None
    with open('./pyproject.toml', 'r') as file:
        content = file.read()
    # Extract the version of openfisca_tunisia
    version_match = re.search(r'^version\s*=\s*"([\d.]*)"', content, re.MULTILINE)
    if version_match:
        openfisca_tunisia = version_match.group(1)
    else:
    ...
    return {
        'openfisca_tunisia': openfisca_tunisia,
...
def replace_in_file(filepath: str, info: dict):
    ...
    meta = meta.replace(PACKAGE_VERSION, info['openfisca_tunisia'])
...
if __name__ == '__main__':
    ...
    if args.only_package_version:
        print(f'{info["openfisca_tunisia"]}')  # noqa: T201
        exit()
