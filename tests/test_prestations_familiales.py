...
Aucune valeur du dispositif ne date de 1960 : la loi n° 60-30 entre en vigueur le 1er avril 1961
pour ses articles 52 et 61 (article 130).
"""
...
def test_les_quatre_ages_du_bareme():
    """Article 61 : taux unique en 1961, barème dégressif en 1976, 4e rang supprimé en 1989."""
    # 1er avril 1961 : taux UNIQUE de 15 %, plafond trimestriel de 52,500 D, quatre rangs.
    a = pf("1961-04-01").af
    assert (a.taux.enf1, a.taux.enf2, a.taux.enf3, a.taux.enf4) == (0.15, 0.15, 0.15, 0.15)
    assert a.plaf_trim == 52.5
    assert a.nb_enfants_max == 4

    # 1976 (loi n° 75-82) : barème dégressif, plafond relevé de 52,500 D à 72 D.
    a = pf("1976-01-01").af
...
def test_le_plafond_de_122_dinars_ne_date_pas_de_1960():
    ...
    assert pf("1961-04-01").af.plaf_trim == 52.5
    assert pf("1975-12-31").af.plaf_trim == 52.5
    assert pf("1976-01-01").af.plaf_trim == 72
...
def test_le_quatrieme_rang_et_le_plancher_sont_cloture_aux_bonnes_dates():
    """Le quatrième rang n'existe que sur une période bornée ; hors d'elle, il est absent."""
    import pytest
...
def test_rien_avant_le_1er_avril_1961():
    """Article 130 : les articles 52 et 61 entrent en vigueur le 1er avril 1961."""
    import pytest
    from openfisca_core.errors import ParameterNotFoundError

    for chemin in ("plaf_trim", "taux.enf1", "taux.enf4", "nb_enfants_max"):
        with pytest.raises(ParameterNotFoundError):
            noeud = pf("1961-03-31").af
            for morceau in chemin.split("."):
                noeud = getattr(noeud, morceau)


def test_article_61_d_origine_plafond_unique_sans_plancher():
    """Art. 61 al. 2 d'origine : plafond unique de 52,500 D par trimestre, aucun plancher."""
    import pytest
    from openfisca_core.errors import ParameterNotFoundError

    for date in ("1961-04-01", "1975-12-31", "1976-01-01", "1989-01-01"):
        with pytest.raises(ParameterNotFoundError):
            pf(date).af.plancher_trim
